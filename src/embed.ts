/*
This class is accessible as window.tipLink
*/
...
import type {
  ...
  export interface TipLinkEmbedEvents {
  ...
  const IFRAME_ID_PREFIX = "tipLinkIframe";
  ...
  function getWidgetUIMetadata(widget: HTMLDivElement, x: number) {
    ...
    const isCollapsedUi = widget.classList.contains("tipLinkWidget_collapsed");
  ...
  function setupWalletWidget({
    ...
    const widgetElementId = widgetId(tipLinkSessionId);
    ...
    const interactable = interact(`#${widgetElementId}`).draggable({
      ...
      onmove: function (event: { target: any; dx: any; dy: any }) {
        onDragStart();
        const target = event.target;
        const isCollapsed = target.classList.contains("tipLinkWidget_collapsed");
  ...
  export class TipLinkEmbed extends EventEmitter<TipLinkEmbedEvents> {
    publicKeyString: string | undefined;
  
    private tipLinkAlertContainer: HTMLDivElement | undefined;
    private tipLinkToastContainer: HTMLDivElement | undefined;
  
    private tipLinkIframe: HTMLIFrameElement | undefined;
    private tipLinkDraggableWidget: Element | undefined;
    private draggableWidgetWindowResizeListener: () => void;
    private draggableWidgetWindowScrollListener: () => void;
    private readonly buildEnv: TIPLINK_BUILD_ENV_TYPE;
    ...
    private readonly tipLinkSessionId: string;
    ...
    constructor(
      title: string,
      buildEnv: TIPLINK_BUILD_ENV_TYPE,
      clientId: string,
      forceIframe: boolean,
      ...
      const prevPublicKey = localStorage.getItem("tipLink_pk_connected");
      ...
      this.tipLinkSessionId = uuid();
      this._walletAdapterNetwork = walletAdapterNetwork;
      this.draggableWidgetWindowResizeListener = () => {
        if (this._isDragging || !this.tipLinkDraggableWidget) return;
        const target = this.tipLinkDraggableWidget as HTMLDivElement;
      ...
      checkAndAttachTipLinkInstance(this);
    }
    ...
    private hideIframe() {
      if (this.tipLinkIframe?.style?.display) {
        this.tipLinkIframe.style.display = "none";
      }
    }
    ...
    private getWhiteTipLinkLogoUrl(): string {
      const logoUrl = new URL(
    ...
    private getDarkTipLinkLogoUrl(): string {
      const logoUrl = new URL(
    ...
    private popupTypeMessage = (popupType: PopupType) => {
      const baseUrl = getTipLinkUrl(this.buildEnv);
      switch (popupType) {
        ...
        case PopupType.NOT_ALLOWLISTED:
          return {
            ...
            description:
              "Your application has not been allowlisted for the TipLink Wallet Adapter. Please reach out to contact@tiplink.io for support",
          };
      }
    };
    ...
    private _showSuccessToast(text: string): void {
      const innerContainer = htmlToElement<HTMLDivElement>(
        `<div id='tipLinkSuccessToast__inner_container'>
          <img id="tipLinkSuccessCheckmark__logo" src="${this.getGreenCheckmarkLogoUrl()}" />
          <p>${text}</p>
        </div>`
      );
      const removeToast = (element?: HTMLDivElement) => {
        if (element) {
          ...
          if (
            ...
            this.tipLinkToastContainer.children.length === 0
          ) {
            this.tipLinkToastContainer.style.display = "none";
          }
        }
      };
      const attachOnLoad = () => {
        if (this.tipLinkToastContainer) {
          innerContainer.addEventListener("click", () => {
            removeToast(innerContainer);
          });
          this.tipLinkToastContainer.appendChild(innerContainer);
        }
      };
      attachOnLoad();
      if (this.tipLinkToastContainer) {
        this.tipLinkToastContainer.style.display = "block";
    ...
    private showTipLinkAutoconnectToast = () => {
      this._showSuccessToast("Your TipLink wallet is connected!");
    };
    ...
    private async _createPopupBlockAlert(
      ...
      const innerContainer = htmlToElement<HTMLDivElement>(
        "<div id='tipLinkAlert__inner_container'></div>"
      );
      ...
      const tipLinkMessageAndButtonContainer = htmlToElement<HTMLDivElement>(
        '<div id="tipLinkAlert__message_btn_container">' +
          `<div id="tipLinkAlert__message_container">` +
          `${
            popupImgUrl
              ? `<div id="tipLinkLogo_container"><img id="tipLinkAlert_logo" src="${popupImgUrl}"></img></div>`
              : ""
          }` +
          `<h1 id="tipLinkAlert__title">${title}</h1>` +
          `${
            description ? `<p id="tipLinkAlert__desc">${description}</p>` : ""
          }` +
          "</div>" +
          "</div>"
      );
      ...
      const successAlert = htmlToElement(
        `<div><button id="tipLinkAlert__btn">${buttonText}</button></div>`
      );
      const btnContainer = htmlToElement(
        '<div id="tipLinkAlert__btn-container"></div>'
      );
    ...
    async init({
      ...
      if (directConnect && !promptTipLinkAutoConnectFromRedirect) {
        windowParams = this.windowCommunicator.openPopup(
          `/embedded_adapter_login?ref=${window.location.origin}${        //Note this is how this how tiplink is connecting 
      ...
      console.log("1212121",tipLinkUrl)
      this.tipLinkIframe = htmlToElement<HTMLIFrameElement>(
        `<iframe
          id="${iframeId(this.tipLinkSessionId)}"
          class="tipLinkIframe"
      ...
      this.tipLinkAlertContainer = htmlToElement<HTMLDivElement>(
        `<div id="tipLinkAlertContainer" style="display:none; z-index: 2147483647"></div>`
      );
  
      this.tipLinkToastContainer = htmlToElement<HTMLDivElement>(
        `<div id="tipLinkToastContainer" style="display:none; z-index: 2147483647"></div>`
      );
    ...
    notifyDisallowed() {
      ...
      showDialog(
        ...
        `<p>${window.location.origin} does not have access yet ` +
          ...
          'TipLink team at <a style="text-decoration: underline;" href="mailto:contact@tiplink.io" target="_blank"> ' +
