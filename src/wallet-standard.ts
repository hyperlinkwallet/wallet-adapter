/**
 * NOTE: This file is forked from https://github.com/anza-xyz/wallet-standard/blob/2c354cf07daa1d440ba9631fcefc5c00b07aa9dd/packages/wallet-adapter/base/src/wallet.ts#L233
 *
 * We needed to slightly modify the auto connect logic, hence the fork and change.
 * Please see below for the modification
 */
...
import {
  ...
  function walletAccountsEqual(a?: WalletAccount, b?: WalletAccount): boolean {
    if (!a && !b) {
      return true;
    }
    if (!a || !b) {
  ...
  export class SolanaWalletAdapterWallet implements Wallet {
    ...
    constructor(adapter: Adapter, chain: SolanaChain, endpoint?: string) {
      ...
      const supportedTransactionVersions = [
        ...(adapter.supportedTransactionVersions || ["legacy"]),
      ];
    ...
    #connect: StandardConnectMethod = async ({ silent } = {}) => {
      if (!this.#adapter.connected) {
        // Here is the modification of the connect logic. If it's silent,
        // we trigger an autoconnect instead (no window being forced open)
        if (silent) {
    ...
    #deserializeTransaction(
      ...
      if (
        ...
        arraysEqual(this.#supportedTransactionVersions, ["legacy"])
      )
    ...
    #signAndSendTransaction: SolanaSignAndSendTransactionMethod = async (
      ...
      if (inputs.length === 1) {
        ...
        if (!walletAccountsEqual(input.account, this.#account))
    ...
    #signTransaction: SolanaSignTransactionMethod = async (...inputs) => {
      ...
      if (inputs.length === 1) {
        ...
        const signedTransactions = await this.#adapter.signAllTransactions(
          transactions
        );
    ...
    #signMessage: SolanaSignMessageMethod = async (...inputs) => {
      ...
      if (inputs.length === 1) {
        ...
        const input = inputs[0]!;
        if (!walletAccountsEqual(input.account, this.#account)) {
          console.error("input account", input.account);
          console.error("this account", this.#account);
          throw new Error("invalid account");
        }
    ...
    #signIn: SolanaSignInMethod = async (...inputs) => {
      ...
      if (inputs.length > 1) {
        ...
        for (const input of inputs) {
          const output = await this.#adapter.signIn(input);
          outputs.push(output);
        }
        return outputs;
      } else {
        const output = await this.#adapter.signIn(inputs[0]);
        return [output];
      }
    };
  }
